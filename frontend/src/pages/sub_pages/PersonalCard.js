...
const PersonalCard = ({vehicle}) => {
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      ...
      if(response.ok){
        dispatch({type: 'GET_PERSONAL_VEHICLES', payload: json})
      }
  ...
  return (
    ...
    <div>
      ...
      <Navbar collapseOnSelect expand="lg" variant="dark" sticky="top" className='nav-bar-edit'>
        <Container>
          <Navbar.Brand>Vehicles - Personal</Navbar.Brand>
      ...
      <h1 className='table-name'>
        ...
        <h2 className='table-info'>
          Executive and managers vehicles management.
        </h2>
